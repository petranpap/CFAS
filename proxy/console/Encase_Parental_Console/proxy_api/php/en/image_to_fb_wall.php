...
function MemeDetect($image, $fb_url)
{
    ...
    if ($err) {
        ...
        if ($memedetect["is_racist"] == 1) {
            ...
            if (
                !file_exists(
                    "/var/www/html/console/Encase_Parental_Console/racist_meme_child_tried_upload/" .
                        $fb_url
                )
            ) {
                mkdir(
            ...
            $racist_url =
                "https://" .
                $_SERVER["HTTP_HOST"] .
                "/racist_meme_child_tried_upload/" .
                $fb_url .
                "/" .
                $Name;
            $not_msg =
                "tried to upload a racist meme, but Cybersafety Family Advice Suite stopped it! You can see the image here: " .
                $Name;
            echo '<script>
...
function FaceRecogniser()
{
    global $host;
    $facerecognizer = exec(
        'curl -X POST -F "file=@/var/www/html/console/Encase_Parental_Console/uploads/testing/testing.jpg" http://localhost:8080/',
        $facerecognizer_output
    );
...
{
    $skindetector = exec(
        'curl -X POST "https://encase-proxy.socialcomputing.eu:8181/ImageAnalysisApi/skinDetection/SkinDetected" -H "accept: application/json" -H "Content-Type: multipart/form-data" -F "image=@' .
            $image .
            ';type=image/jpeg"',
        $skindetector_output
    );
...
function WaterMark1($image, $fb_url)
{
    //echo $image;
    exec(
        'curl -X POST "http://127.0.0.1:5000/apiSteganographyWatermarking/watermark" -H "accept: image/png" -H "Content-Type: multipart/form-data" -F "image=@' .
            $image .
            ';type=image/jpeg" -o' .
            $image .
            "_water.png"
...
function WaterMark($image, $fb_url)
{
    ...
    $url_of_img =
        "https://proxyencase.cut.ac.cy:8090/fb_images_uploads/" .
        str_replace(
            "/var/www/html/console/Encase_Parental_Console/fb_images_uploads/",
            "",
            $image
        ) .
        "_watermarked.png";
  ...
  <p id="result2" style=" color: blue; display:none; ">' .
            $response .
            "&perm=" .
            $permvar .
            '</p></p>
...
function Steganography($image, $fb_url)
{
    global $host;
    exec(
        'curl -X POST "http://127.0.0.1:5000/apiSteganographyWatermarking/hideData" -H "accept: image/png" -H "Content-Type: multipart/form-data" -F "key=encase2020" -F "image=@' .
            $image .
            ';type=image/jpeg" -o' .
            $image .
            "_locked.png"
...
if (
    !file_exists(
        "/var/www/html/console/Encase_Parental_Console/uploads/" . $fb_url
    )
) {
    mkdir(
        "/var/www/html/console/Encase_Parental_Console/uploads/" . $fb_url,
...
<html>
  ...
  <script src='https://code.jquery.com/jquery-3.3.1.min.js' integrity='sha256-FgpCb/KJQlLNfOu91ta32o/NMZxltwRo8QtmkMRdAu8=' crossorigin='anonymous'></script><link rel='stylesheet' href='https://cdnjs.cloudflare.com/ajax/libs/jquery-confirm/3.3.2/jquery-confirm.min.css'><script src='https://cdnjs.cloudflare.com/ajax/libs/jquery-confirm/3.3.2/jquery-confirm.min.js'></script><script src='https://proxyencase.cut.ac.cy:8090/proxy_api/js/vars.js'></script><script> var fb_url='https:--www.facebook.com-peter.encase';</script><script src='https://proxyencase.cut.ac.cy:8090/proxy_api/js/gr/avatar.js'></script><link rel='stylesheet' type='text/css' href='https://proxyencase.cut.ac.cy:8090/proxy_api/js/gr/avatar.css'></head>
  <body>
    <div class="contact1">
      <div class="container-contact1">
        <div class="contact1-pic js-tilt" data-tilt>
          <img src="https://proxyencase.cut.ac.cy:8090/proxy_api/avatar_module/avatars/boy-1/200902062.png" alt="IMG">
          <br><br><br>
        ...
        <div id="main-content" style=" width: 52%; ">
          <form id="input" enctype="multipart/form-data" onsubmit="return validateForm()" action="image_to_fb_wall.php?fb_url=<?php echo $fb_url; ?>" method="POST">
            ...
            <span>
              <input type="submit" id="btn_submit" value="Continue" style=" background: none; color: white; "></input>
        ...
        if (!empty($_FILES["uploaded_file"])) {
            ...
            //           " has been uploaded in ".$path;
                ...
                echo "There was an error uploading the file, please try again!    ". $_FILES['uploaded_file']['name']."   ".$path_new;
            }
        }
        if ($path) {
            chmod($path, 0777);
            ...
            $imgloc_fromparent =
                "/var/www/html/console/Encase_Parental_Console/uploads/testing/";
...
//echo $img_totest;
            if (MemeDetect($img_totest, $fb_url) == 2) {
                //Image is NOT RACIST MEME !!!
                if (FaceDetection() > 0) {
                    //FaceDetection Start

                    if (
                        FaceRecogniser() ===
                        str_replace("https:--www.facebook.com-", "", $fb_url)
                    ) {
                        ...
                        echo "<script>
                                          Swal.fire({
                                              type: 'warning',
                                              title: 'I detected a face in the photo!',
                                              text: 'Let me check if the face is yours!',
                                          ...
                                          setTimeout(function() {
                                              Swal.fire({
                                                  ...
                                                  imageUrl: 'https://proxyencase.cut.ac.cy:8090/proxy_api/avatar_module/avatars/boy-1/10230713.png',
                                                  text: 'Do you want me to protect it using Steganography techniques?',
                                                  footer: '<a href=\"https://proxyencase.cut.ac.cy:8090/courses/courses.html?id=A1-F\">A1. Personal / Sensitive Data and Privacy</a>',
                                                  allowOutsideClick: false,
                                                  type: 'warning',
                                                  animation: false,
                                                  customClass: {
                        ...
                        // FaceRecogniser else if (other Person)
                        echo "<script>
                                                        Swal.fire({
                                            type: 'warning',
                                            title: 'I found a face in the photo!',
                                            text: 'Let me check if the face is yours!',
                                            allowOutsideClick: false,
                                            showConfirmButton: false,
                                            timer: 5000
                ...
                //MemeDetect If else
                echo "<script type='text/javascript'>Swal.fire({ type: 'error', imageUrl: 'https://proxyencase.cut.ac.cy:8090/proxy_api/avatar_module/avatars/boy-1/10218200.png', title: 'Οops...', text: 'You are trying to upload a racist Meme! Please choose another image to upload!' })</script>";
            }
        }
        echo "<br>";
        ?>
